...
maxD=$(sed '1,1d' ${out_dir}/${out_name}_Zeta_anno.txt|awk 'BEGIN{FS=OFS="\t"}{if($4=="non-exp" || $4=="Gene"){print $2}}'|sort -g -r |sed -n '20,20p')
maxI=$(sed '1,1d' ${out_dir}/${out_name}_Zeta_anno.txt|awk 'BEGIN{FS=OFS="\t"}{if($4=="non-exp" || $4=="Gene"){print $3}}'|sort -g -r |sed -n '20,20p')
