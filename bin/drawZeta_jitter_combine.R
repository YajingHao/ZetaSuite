...
data<-data[data$SS<1,]
pdf(args[4],height=4,width=8)
ggplot(data,aes(x=Cut.Off,y=SS,col=Type))+geom_point()+geom_smooth(span=0.2)+theme_bw()+xlab("Zeta Score")+ylab("Screen strength")+theme(legend.position = c(0.8, 0.2),legend.title=element_blank())
dev.off()
