...
SVMcurve<-read.table("SVM_lineCutOff_I",sep="\t")
Zseq_1<-temp$Zseq_I
temp_I<-0
...
if(((lengthUse_I/length(Zscore[num,]))+(lengthUse_I_add/length(Zscore[num,]))-SVMcurve[j,2]-SVMcurve[j+1,2])*Zseq[j+1]*(Zseq[j+1]-Zseq[j])/2 >0 )
{
tempI<-tempI+((lengthUse_I/length(Zscore[num,]))+(lengthUse_I_add/length(Zscore[num,]))-SVMcurve[j,2]-SVMcurve[j+1,2])*Zseq[j+1]*(Zseq[j+1]-Zseq[j])/2
}
}
return (temp_I)
}
...
stopCluster(cl)
write.table(outputdata_I,args[3],sep="\t",row.names=T,quote=F,col.names=T)
